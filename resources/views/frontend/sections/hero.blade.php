                ...
                <div class="bg-white/10 backdrop-blur-sm rounded-2xl p-8 border border-white/20 shadow-2xl">
                    ...
                    <div class="hero-subtitle text-lg text-[#8892b0] mb-8 fade-in leading-relaxed">
                        <div class="hero-subtitle max-w-xl mx-auto text-center text-[#8892b0] leading-relaxed">
                            {!! Str::of($hero->subtitle)->replace(['<h1', '<h2' ], '<p' )->replace(['</h1>', '</h2>'], '</p>') !!}
                        </div>


                    </div>
                    @if($hero->button_text != null)
                    <div class="flex flex-wrap gap-4">
                        <a href="{{ $hero->button_link }}" class="bg-gradient-to-r from-[#00b4d8] to-[#ff5d8f] text-white font-bold py-3 px-6 rounded-full flex items-center gap-2 hover:shadow-xl hover:-translate-y-1 transition-all duration-300 pulse" data-ar="اكتشف المزيد" data-en="Discover More">
