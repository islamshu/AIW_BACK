...
{{-- =====================================================
   SECTION / REPEATER MANAGEMENT
===================================================== --}}
<script>
document.addEventListener('click', function (e) {

    /* فتح مودال إضافة قسم */
    const openAddSection = e.target.closest('.js-open-add-section');
    if (openAddSection) {
        document.getElementById('addSectionLayoutId').value = openAddSection.dataset.layout;
        document.getElementById('addSectionColIndex').value = openAddSection.dataset.col;
    ...
    /* حذف قسم */
    const deleteSection = e.target.closest('.js-mark-delete');
    if (deleteSection) {
        if (!confirm('هل أنت متأكد من حذف هذا القسم؟')) return;
        const chip = deleteSection.closest('.js-section-chip');
...
{{-- =====================================================
   CKEDITOR + MEDIA LIBRARY + IMAGE RESIZE
===================================================== --}}
...
function initEditors() {
    ...
    document.querySelectorAll('.js-editor:not(.editor-loaded)').forEach(el => {
...
function addMediaLibraryButton(editor) {
    setTimeout(() => {
        ...
        if (!toolbar || toolbar.querySelector('.ck-media-library-btn')) return;

        const btn = document.createElement('button');
        btn.className = 'ck-media-library-btn';
        btn.type = 'button'; // 🔥 مهم جدًا
        btn.innerHTML = '📁 مكتبة الوسائط';
        ...
        toolbar.querySelector('.ck-toolbar__items').appendChild(btn);
    }, 300);
}

window.addEventListener('message', function (event) {
    if (event.data?.type !== 'insert-image-editor') return;
    if (!activeEditorInstance) return;

    activeEditorInstance.model.change(writer => {
        const image = writer.createElement('imageBlock', { src: event.data.media.url });
        activeEditorInstance.model.insertContent(image);
    });
});
...
function uploadImagePlugin(editor) {
    editor.plugins.get('FileRepository').createUploadAdapter = loader => ({
        upload: () => loader.file.then(file => new Promise(resolve => {
            ...
            xhr.onload = () => resolve({ default: JSON.parse(xhr.response).url });
...
document.addEventListener('DOMContentLoaded', () => {
    initEditors();
    new MutationObserver(initEditors).observe(document.body,{childList:true,subtree:true});
});

document.addEventListener('submit', () => {
    editors.forEach(e => e.sourceElement.value = e.getData());
});
</script>
