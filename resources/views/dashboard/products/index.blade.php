        ...
        $(document).ready(function() {
            ...
            $('.filter-toggle').on('click', function() {
                ...
                filterBody.slideToggle(300, function() {
                    // هذه الدالة تنفذ بعد انتهاء الحركة
                    if (filterBody.is(':visible')) {
                        filterBox.removeClass('filter-collapsed').addClass('filter-expanded');
                        $(this).find('.filter-toggle').html(
                            '{{ __('إخفاء') }} <i class="ft-chevron-up"></i>');
                    } else {
                        filterBox.removeClass('filter-expanded').addClass('filter-collapsed');
                        $(this).find('.filter-toggle').html(
                            '{{ __('عرض') }} <i class="ft-chevron-down"></i>');
                    }
                });

                // تغيير الأيقونة بشكل فوري
                icon.toggleClass('ft-chevron-up ft-chevron-down');
