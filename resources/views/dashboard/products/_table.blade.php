    ...
    <tbody>
        @forelse ($products as $product)
            <tr>
                ...
                <td><img src="{{ asset('storage/' . $product->image) }}" width="60" alt="{{ $product->name }}"
                ...
                <td>{{ $product->reviews->count() }} @if ($product->reviews->count() > 0)
                        <a class="btn btn-info"
                            href="{{ route('product_rating', $product->id) }}">{{ __('مشاهدة التقييمات') }}</a>
                    @endif
                </td>
                <td>
